...
import { Search, Clock, CheckCircle, Layers, Users, Calendar, ChevronRight } from 'lucide-react';
    ...
    <div className="h-full overflow-y-auto bg-slate-50/50">
      <div className="p-8 max-w-7xl mx-auto space-y-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <h1 className="text-3xl font-black text-slate-800">NuraTask Dashboard</h1>
            <p className="text-slate-500 text-sm">Real-time productivity overview</p>
          </div>
            ...
            {searchQuery && (
              <div className="absolute top-full mt-2 w-full bg-white border border-slate-200 rounded-xl shadow-xl z-50 overflow-hidden">
                {filteredResults.map(res => (
                  <div key={res.id} className="p-3 hover:bg-slate-50 cursor-pointer border-b border-slate-50 last:border-none flex justify-between items-center">
                    <span className="text-sm font-bold text-slate-700">{res.title}</span>
                    <span className="text-[10px] bg-slate-100 px-2 py-0.5 rounded uppercase">{res.boardTitle}</span>
                  </div>
                ))}
              </div>
            )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          <StatCard label="Time Logged" value={`${(allData.stats.totalTime / 3600).toFixed(1)}h`} icon={<Clock />} color="blue" />
          <StatCard label="Tasks Completed" value={allData.stats.completed} icon={<CheckCircle />} color="emerald" />
          <StatCard label="Team Members" value={members.length} icon={<Users />} color="amber" />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 bg-white p-6 rounded-2xl border border-slate-100 shadow-sm">
            <div className="mb-6">
              <h2 className="text-lg font-bold text-slate-800">Workflow Distribution</h2>
              <p className="text-xs text-slate-400">Total tasks by list status</p>
            </div>
          ...
          <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm">
            ...
            <div className="space-y-4">
              {allData.upcoming.map(card => (
                <div key={card.id} className="group p-3 hover:bg-slate-50 rounded-xl transition-all border border-transparent hover:border-slate-100">
                  <div className="flex justify-between items-start mb-1">
                    <span className="text-sm font-bold text-slate-700 truncate pr-2">{card.title}</span>
                    <span className="text-[10px] font-black text-blue-600 bg-blue-50 px-2 py-0.5 rounded">
                      {new Date(card.dueDate).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}
                    </span>
                  </div>
                  <div className="flex items-center gap-1.5">
                    <div className={`w-1.5 h-1.5 rounded-full ${card.boardColor}`} />
                    <span className="text-[10px] text-slate-400 uppercase font-bold tracking-tight">{card.boardTitle}</span>
                  </div>
                </div>
              ))}
            </div>
